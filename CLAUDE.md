...
이 프로젝트는 **내부 전용 단순 진료보고서 생성 도구**다. 기술적 완성도나 최신 스택 적용이 목표가 아니다.

- 현재 잘 동작하는 코드·설정·구조는 건드리지 않는다.
- `tsconfig.json`, `package.json`, `tailwind.config.js` 등 설정 파일은 사용자가 명시적으로 요청하지 않는 한 절대 수정하지 않는다.
- "최신 스택", "모범 사례", "권장 설정"을 이유로 기존 설정을 바꾸지 않는다.
- 라이브러리 업그레이드, 빌드 도구 교체, 아키텍처 개편 등을 자의적으로 제안하거나 적용하지 않는다.
- 요청한 기능만 구현한다. 불필요한 추상화, 유틸리티, 헬퍼를 추가하지 않는다.
